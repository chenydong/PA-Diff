...
if __name__ == '__main__':
    ...
    img = torch.zeros(2, 9, 256, 256)
    img_t = torch.zeros(2, 1, 256, 256)
    time = torch.tensor([1, 2])
    model = UNet(inner_channel=48, norm_groups=24, in_channel=9)
    output, a = model(img, time, img_t)
    # output = model2(img)
    print(output.shape)
    print(a.shape)