...
class ResnetBloc_eca(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None, norm_groups=32, dropout=0, with_attn=False):
        ...
        if with_attn:
            self.attn = TransformerBlock_eca(dim=int(dim), num_heads=2, ffn_expansion_factor=2.66,
                               bias=False, LayerNorm_type='WithBias')

    def forward(self, x, time_emb, p_a, p_t):
        x = self.res_block(x, time_emb)
        if self.with_attn :
            x = self.attn(x, p_a,p_t)
        return x
...
class Encoder(nn.Module):
    ...
    def forward(self, x, t, p_a, p_t):
        x = self.conv1(x)
        x1 = self.block1(x, t, p_a, p_t)

        x2 = self.conv2(x1)
        x2 = self.block2(x2, t, p_a, p_t)

        x3 = self.conv3(x2)
        x3 = self.block3(x3, t, p_a, p_t)

        x4 = self.conv4(x3)
        x4 = self.block4(x4, t, p_a, p_t)

        de_level3 = self.conv_up3(x4)
        de_level3 = torch.cat([de_level3, x3], 1)
        de_level3 = self.conv_cat3(de_level3)
        de_level3 = self.decoder_block3(de_level3, t, p_a, p_t)

        de_level2 = self.conv_up2(de_level3)
        de_level2 = torch.cat([de_level2, x2], 1)
        de_level2 = self.conv_cat2(de_level2)
        de_level2 = self.decoder_block2(de_level2, t, p_a, p_t)

        de_level1 = self.conv_up1(de_level2)
        de_level1 = torch.cat([de_level1, x1], 1)
        mid_feat = self.decoder_block1(de_level1, t, p_a, p_t)
...
class UNet(nn.Module):
    ...
    def forward(self, x, time, p_a, p_t):

        t = self.time_mlp(time) if exists(self.time_mlp) else None

        mid_feat, x1 = self.encoder_water(x, t, p_a, p_t)

        mid_feat2 = self.refine(mid_feat, t, p_a, p_t)
